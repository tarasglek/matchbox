...
import sys
"""
cat bootkube-controller.yaml | ./cp_yaml.py --group etcd --user etcd `find assets/tls/etcd-* assets/tls/etcd` /etc/ssl/etcd/ | ./cp_yaml.py assets/auth/kubeconfig /etc/kubernetes/kubeconfig | ./cp_yaml.py --fullname `find assets/` /opt/bootkube/ |sed 's/^#/ /' > ../examples/ignition/bootkube-controller.yaml
